...
class OCVWorkChain(ProtocolMixin, WorkChain): # maybe BaseRestartWorkChain?
    ...
    def get_builder_from_protocol(
        cls, code, structure, bulk_cation_structure=None, discharged_unitcell_relaxed=None, charged_unitcell_relaxed=None, protocol=None, overrides=None, **kwargs
    ):
        """
    ...
    def run_relax_discharged(self):
        ...
        if self.inputs.get('discharged_unitcell_relaxed'):
            ...
            self.inputs.metadata.call_link_label = 'discharged_base'
            inputs.metadata.label = 'discharged_base'
    ...
    def run_relax_charged(self):
        ...
        if not self.ctx.discharged_unitcell_relaxed: # better to use if inputs.get('discharged_unitcell_relaxed)?
            try:
        ...
        if self.inputs.get('charged_unitcell_relaxed'):
            ...
            # Set the `CALL` link label
            self.inputs.metadata.call_link_label = 'charged_base'
            inputs.metadata.label = 'charged_base'
    ...
    def build_supercells(self):
        ...
        volume_change = abs(volume_discharged - volume_charged) / volume_discharged
        if volume_change > threshold:
            self.report('The Volume changed too much upon cation removal')
            self.exit_codes.ERROR_MECHANICAL_UNSTABLE
        else: 
